...
public class WinMSSStageRepository {
	...
	public static List<Stage> findStagesForMatch(Long matchId) {
		List<Stage> stages = new ArrayList<Stage>();
		Connection connection = WinMssDatabaseUtil.getConnection();		
		Statement statement = null;
		ResultSet resultSet = null;
		try {
			statement = connection.createStatement();
			resultSet = statement.executeQuery("SELECT MatchId, StageId, StageName FROM tblMatchStage WHERE MatchId=" + matchId);
			while (resultSet.next()) {
				Stage stage = new Stage();
				stage.setWinMssMatchId(resultSet.getLong(1));
				stage.setWinMssId(resultSet.getLong(2));
				stage.setName(resultSet.getString(3));
				stage.setStageScoreSheets(WinMSSStageScoreSheetRepository.findStageScoreSheetsForStage(matchId, stage.getWinMssId()));
				stages.add(stage);
			}
			resultSet.close();
			statement.close();
